    ...
    def isBalanced(self, root: Optional[TreeNode]) -> bool:
        ...
        def depthIsEqual(parent: Optional[TreeNode]) -> int: 
            ...
            if not parent.left and not parent.right:
                depthOfTree[parent.val] = 1
                return True
            elif not parent.left and parent.right:
                balanced = depthIsEqual(parent.right) 
                depthOfTree[parent.val] = 1 + depthOfTree[parent.right.val]
                return balanced and depthOfTree[parent.right.val] <= 1
            elif not parent.right and parent.left:
                balanced = depthIsEqual(parent.left)
                depthOfTree[parent.val] = 1 + depthOfTree[parent.left.val]
                return balanced and depthOfTree[parent.left.val] <= 1
            else:
                balanced = depthIsEqual(parent.left) and depthIsEqual(parent.right)
                depthOfTree[parent.val] = 1 + max(depthOfTree[parent.left.val], depthOfTree[parent.right.val])
                return balanced and abs(depthOfTree[parent.right.val] - depthOfTree[parent.left.val]) <= 1
